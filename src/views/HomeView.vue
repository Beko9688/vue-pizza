      ...
      <div class="content__wrapper">
        ...
        <div class="content__pizza">
          ...
          <pizza-constructor
              ...
              :ingredients="pizzaIngredientsForView"
...
const pizzaIngredientsForView = computed(() => {
  return pizzaStore.ingredientsExtended.reduce((acc, ingredient) => {
    if (!ingredient.image) {
      return acc;
    }
    ...
    acc[slug] = ingredient.quantity ?? 0;
    return acc;
  }, {});
});

const onDropIngredient = (slug) => {
  const ingredient = dataStore.ingredients.find((item) => {
    if (!item.image) return false;

    const itemSlug = item.image
        .split("/")
        .pop()
        .replace(".svg", "");
  ...
  pizzaStore.incrementIngredientQuantity(ingredient.id);
};
